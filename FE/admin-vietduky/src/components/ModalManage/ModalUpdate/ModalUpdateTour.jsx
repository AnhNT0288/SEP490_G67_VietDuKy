...
import {
  ...
  updateTour,
  getTourById,
} from "../../../services/API/tour.service.js";
import Select from "react-select";
import { toast } from "react-toastify";

// eslint-disable-next-line react/prop-types
export default function ModalUpdateTour({
  mode = "update",
  tourId = null,
  onClose,
  onCreateSuccess,
  ...
  const handleDeleteActivity = (activityId) => {
    ...
    const confirm = window.confirm(
      "Bạn có chắc chắn muốn xoá chương trình này?"
    );
    if (!confirm) return;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (id) {
        toast.success(
          `${mode === "update" ? "Cập nhật" : "Tạo"} Tour thành công!`
        );
        onCreateSuccess?.(id);
        onClose();
      } else {
  ...
  const handleAddActivity = (activitiesAdded) => {
    const newList = Array.isArray(activitiesAdded)
      ? activitiesAdded
      : [activitiesAdded];
    setTourData((prev) => ({
      ...prev,
                ...
                onDrop={(e) => {
                  ...
                  if (newFiles.length > 0) {
                    setTourData((prev) => ({
                      ...prev,
                      album: [
                        ...(Array.isArray(prev.album) ? prev.album : []),
                        ...newFiles,
                      ],
                    }));
                    setPreviewImages((prev) => [
            ...
            <div className="w-3/5">
              <div>
                <div className="flex justify-between items-center mb-4">
                  <div>
                    <label className="font-medium">Chương trình Tour</label>
                  </div>
