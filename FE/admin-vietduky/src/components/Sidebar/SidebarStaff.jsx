...
export default function SidebarStaff({isCollapsed, setBreadcrumb }) {
    ...
    const menuItems = [
        ...
        },
        {
            ...
            onClick: () => {
                ...
                if (user?.id) {
                    navigate(`/staff/${user.id}/assigned-guides`);
                }
            }
        },
        {
    ...
    return (
        <aside className={`fixed md:relative min-h-screen bg-[#9A1B21] text-white p-4 transition-all duration-300 ${isCollapsed ? "w-20" : "w-64"}`}>
                            ...
                            <div className={`flex items-center justify-between p-2 rounded cursor-pointer ${isSelected ? "bg-[#F4F4F5] text-black" : "hover:bg-[#B22222]"}`}
                                 onClick={() => {
                                     if (item.subItems?.length) {
                                         ...
                                         item.onClick(); // Gọi hàm onClick nếu có
                                         setSelected(item.id);
                                         setOpenSubMenu(null);
