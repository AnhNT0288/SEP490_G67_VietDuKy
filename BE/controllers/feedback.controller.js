...
exports.createFeedbackForTour = async (req, res) => {
    try {
        ...
        if (tourFeedbacks.length > 0) {
            // Tính trung bình cộng rating
            const totalRating = tourFeedbacks.reduce((sum, feedback) => sum + (feedback.rating || 0), 0);
            const averageRating = tourFeedbacks.length > 0 ? (totalRating / tourFeedbacks.length).toFixed(1) : 0;
            tour.rating = averageRating;
        } else {
            tour.rating = feedbackRating;
        }
        await tour.save();
