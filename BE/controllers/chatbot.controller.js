...
exports.askChatBot = async (req, res) => {
    try {
        ...
        // Chỉ lấy lịch sử chat nếu có user_id
        if (user_id) {
            const recentHistory = await db.ChatHistory.findAll({
                where: { user_id },
                order: [['created_at', 'DESC']],
                limit: 5
            });

            history = recentHistory.reverse().map(chat => ({
                role: "user",
                parts: [{ text: chat.question }]
            })).concat(recentHistory.reverse().map(chat => ({
                role: "model",
        ...
        // Chỉ lưu lịch sử chat nếu có user_id
        if (user_id) {
